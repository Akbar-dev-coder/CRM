...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const tsn_app_email = settings['tsn_app_email'];
  const tsn_base_url = settings['tsn_base_url'];

  const url = checkAndCorrectURL(tsn_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | tsn',
    tsn_app_email,
